...
// Step through time and detect passes (satellite above the horizon). Only passes
// whose peak elevation reaches `minEl` degrees are returned. Runs client-side.
export function predictPasses({ line1, line2, lat, lon, alt = 0, days = 2, minEl = 10, stepSec = 30 }) {
  ...
  const end = start + Math.min(days, 5) * 86400 * 1000;
  const passes = [];
  let current = null;
